...
class FusedLocationProviderClient {
  ...
  final Map<int, StreamSubscription> _callbackSubscriptions;
  ...
  FusedLocationProviderClient._create(
    ...
    this._eventChannel,
    this._callbacks,
    this._callbackSubscriptions,
  ) {
    _methodChannel.setMethodCallHandler(_methodCallHandler);
  }

  Future<void> _methodCallHandler(MethodCall methodCall) async {
    print('[FusedLocationProviderClient] _methodCallHandler called: method=${methodCall.method}, args=${methodCall.arguments}');
    switch (methodCall.method) {
      case 'onLocationResult':
        final callbackId = methodCall.arguments['callbackId'];
        print('[FusedLocationProviderClient] onLocationResult: callbackId=$callbackId, hasCallback=${_callbacks.containsKey(callbackId)}');
        if (_callbacks.containsKey(callbackId)) {
          _callbacks[callbackId]!.onLocationResult(
            LocationResult.fromMap(
              methodCall.arguments['locationResult'],
            ),
          );
          print('[FusedLocationProviderClient] onLocationResult callback executed successfully');
        } else {
          print('[FusedLocationProviderClient] ERROR: No callback found for callbackId=$callbackId');
        }
        break;
      case 'onLocationAvailability':
        final callbackId = methodCall.arguments['callbackId'];
        print('[FusedLocationProviderClient] onLocationAvailability: callbackId=$callbackId, hasCallback=${_callbacks.containsKey(callbackId)}');
        if (_callbacks.containsKey(callbackId)) {
          _callbacks[callbackId]!.onLocationAvailability(
            LocationAvailability.fromMap(
              methodCall.arguments['locationAvailability'],
            ),
          );
          print('[FusedLocationProviderClient] onLocationAvailability callback executed successfully');
        } else {
          print('[FusedLocationProviderClient] ERROR: No callback found for callbackId=$callbackId');
        }
        break;
      default:
        print('[FusedLocationProviderClient] Unknown method: ${methodCall.method}');
        break;
    }
  }
  ...
  Future<Location> getLastLocation() async {
    var r= await _methodChannel.invokeMapMethod<String, dynamic>('getLastLocation');
  ...
  Future<int> requestLocationUpdatesCb(
    ...
    final eventChannel = EventChannel(
      'com.huawei.flutter.location/fusedlocation_eventchannel/callback_$callbackId',
    );
    
    final subscription = eventChannel.receiveBroadcastStream().listen((dynamic event) {
      if (event is Map) {
        final type = event['type'];
        final receivedCallbackId = event['callbackId'];
        
        if (receivedCallbackId == callbackId) {
          if (type == 'onLocationResult') {
            locationCallback.onLocationResult(
              LocationResult.fromMap(event['locationResult']),
            );
          } else if (type == 'onLocationAvailability') {
            locationCallback.onLocationAvailability(
              LocationAvailability.fromMap(event['locationAvailability']),
            );
          }
        }
      }
    ...
    _callbackSubscriptions[callbackId] = subscription;
    return callbackId;
  }
  ...
  Future<int> requestLocationUpdatesExCb(
    ...
    print('[FusedLocationProviderClient] requestLocationUpdatesExCb called');
    int callbackId = (await (_methodChannel.invokeMethod<int>(
      'requestLocationUpdatesExCb',
      (locationRequest..priority = LocationRequest.PRIORITY_HD_ACCURACY)
          .toMap(),
    )))!;
    print('[FusedLocationProviderClient] requestLocationUpdatesExCb: received callbackId=$callbackId');
    _callbacks.putIfAbsent(callbackId, () => locationCallback);
    ...
    final subscription = eventChannel.receiveBroadcastStream().listen((dynamic event) {
      print('[FusedLocationProviderClient] EventChannel received event: $event');
      if (event is Map) {
        final type = event['type'];
        final receivedCallbackId = event['callbackId'];
        ...
        if (receivedCallbackId == callbackId) {
          if (type == 'onLocationResult') {
            print('[FusedLocationProviderClient] Processing onLocationResult for callbackId=$callbackId');
            locationCallback.onLocationResult(
              LocationResult.fromMap(event['locationResult']),
            );
          } else if (type == 'onLocationAvailability') {
            print('[FusedLocationProviderClient] Processing onLocationAvailability for callbackId=$callbackId');
            locationCallback.onLocationAvailability(
              LocationAvailability.fromMap(event['locationAvailability']),
            );
          }
      ...
      print('[FusedLocationProviderClient] EventChannel error: $error');
    });
    
    _callbackSubscriptions[callbackId] = subscription;
    print('[FusedLocationProviderClient] requestLocationUpdatesExCb: EventChannel subscription created, total callbacks=${_callbacks.length}');
    return callbackId;
  }
  ...
  Future<void> removeLocationUpdatesCb(int callbackId) async {
    ...
    final subscription = _callbackSubscriptions.remove(callbackId);
